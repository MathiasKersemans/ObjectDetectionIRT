...
def box_filter_based_scores(all_boxes, all_classes, all_scores, scores_threshold):
    boxes_list = []
    classes_list = []
    scores_list = []
    num_TSR = len(all_scores)
    for i in range(0, num_TSR):
        b = []
        c = []
        s = []
        num_box = len(all_scores[i])
        for j in range(0, num_box):
            scores = all_scores[i][j]
            if scores > scores_threshold:
                b.append(all_boxes[i][j])
                c.append(all_classes[i][j])
                s.append(all_scores[i][j])
        boxes_list.extend(b)
